...
class FractionNumber
{
	...
	public:
		...
		{
			FractionNumber ob2;
			float decimalPoint=op2-floor(op2);
			int td=0,newInt=0;
			while(decimalPoint)
			{
				decimalPoint*=(float)10;
				newInt=newInt*10+(int)decimalPoint;
				decimalPoint-=(float)((int)decimalPoint);
				td++;
			}
			int dnum=1;
			while(td--)
...
FractionNumber operator+(float f,FractionNumber& ob)
{
	return ob+f;
}
